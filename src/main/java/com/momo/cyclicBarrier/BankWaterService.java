...
public class BankWaterService implements Runnable{
     ...
     * 保存每个sheet计算出的银流结果
     */
    private ConcurrentHashMap<String,Integer> sheetBankThreadCount
        ...
        for (int i = 0; i < 4; i++) {
            executor.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    sheetBankThreadCount.put(Thread.currentThread().getName(),1);
                    //银流计算完后才能，插入一个屏障
                    try{
                        c.await();
    ...
    public void run() {
        int result = 0;
        //汇总每个sheet计算出的结果
        for (Map.Entry<String,Integer> sheet: sheetBankThreadCount.entrySet()) {
            result += sheet.getValue();
        }
        //将结果输出
        sheetBankThreadCount.put("result",result);
